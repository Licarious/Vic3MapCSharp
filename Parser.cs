...
namespace Vic3MapCSharp
{
    public class Parser
    {
        ...
        public static string? BuildingGroupPrefix { get; private set; }
        ...
        public static Dictionary<string, State> ParseStateFiles(Dictionary<Color, Province> provDict, string localDir)
        {
            ...
            foreach (string file in files)
            {
                ...
                State s = new();
                Resource dr = new();
                bool arabalResourceFound = false, cappedResourceFound = false, discoverableResourceFound = false, traitsFound = false, provincesFound = false;
                ...
                foreach (string line in lines)
                {
                    ...
                    if (arabalResourceFound)
                    {
                        foreach (var res in value.Replace("\"", "").Split().Where(r => r.StartsWith("bg_", StringComparison.OrdinalIgnoreCase) || r.StartsWith("building_", StringComparison.OrdinalIgnoreCase)))
                        {
                            BuildingGroupPrefix ??= res.StartsWith("bg_", StringComparison.OrdinalIgnoreCase) ? "bg_" : "building_";
                    ...
                    if (cappedResourceFound && (cl.TrimStart().StartsWith("bg_", StringComparison.OrdinalIgnoreCase) || cl.TrimStart().StartsWith("building_", StringComparison.OrdinalIgnoreCase)))
                    {
                        var l2 = cl.Replace("\"", "").Split("=");
                        var resourceKey = l2[0].Trim();

                        BuildingGroupPrefix ??= resourceKey.StartsWith("bg_", StringComparison.OrdinalIgnoreCase) ? "bg_" : "building_";

                        s.Resources[resourceKey] = new Resource(resourceKey)
                        {
                    ...
                    if (discoverableResourceFound)
                    {
                        ...
                        {
                            var resourceName = l2[1].Trim().Replace("\"", "");

                            if (resourceName.StartsWith("bg_", StringComparison.OrdinalIgnoreCase) || resourceName.StartsWith("building_", StringComparison.OrdinalIgnoreCase))
                            {
                                BuildingGroupPrefix ??= resourceName.StartsWith("bg_", StringComparison.OrdinalIgnoreCase) ? "bg_" : "building_";
                            }

                            dr = new Resource(resourceName)
                            {
                                Type = "discoverable"
                            };
        ...
        public static void ParseRGOsCSV(Dictionary<string, State> states, string localDir)
        {
            ...
            for (int i = 1; i < lines.Length; i++)
            {
                ...
                {
                    string resourceName = BuildingGroupPrefix + header[j].Replace("Known ", "");
                ...
                {
                    string resourceName = BuildingGroupPrefix + ar;
                    state.Resources[resourceName] = new Resource(resourceName)
                    {
        ...
        /// <summary>
        /// Parses diplomacy files and populates diplomatic pacts for nations.
        /// </summary>
        /// <param name="nations">Dictionary of nations keyed by nation tag.</param>
        /// <param name="localDir">Local directory path.</param>
        public static void ParseDiplomacyFiles(Dictionary<string, Nation> nations, string localDir)
        {
            ArgumentNullException.ThrowIfNull(nations);
            ArgumentNullException.ThrowIfNull(localDir);

            string filePath = Path.Combine(localDir, "_Input", "common", "history", "diplomacy");

            //for each file in directory
            foreach (string file in Directory.GetFiles(filePath, "*.txt"))
            {
                string[] lines = File.ReadAllLines(file);
                Nation? overlordNation = null;
                int indent = 0;
                Boolean isDiplomaticPact = false;
                Nation? subjectNation = null;
                String pactType = "";
                foreach (string line in lines)
                {
                    if (string.IsNullOrWhiteSpace(line) || line.StartsWith('#')) continue;
                    string cl = CleanLine(line);
                    if (indent == 1 && cl.Contains('='))
                    {
                        string nationTag = cl.Split('=')[0].Replace("c:", "").Replace("?","").Trim();
                        nations.TryGetValue(nationTag, out overlordNation);
                    }
                    else if (indent == 2 && overlordNation != null)
                    {
                        if (cl.Contains("create_diplomatic_pact")) isDiplomaticPact = true;
                    }
                    if (isDiplomaticPact)
                    {
                        if (cl.Contains("country"))
                        {
                            string nationTag = cl.Split('=')[1].Replace("c:", "").Trim();
                            nations.TryGetValue(nationTag, out subjectNation);
                        }
                        else if (cl.Contains("type"))
                        {
                            pactType = cl.Split('=')[1].Trim();
                        }
                    }
                    if (cl.Contains('{') || cl.Contains('}'))
                    {
                        ...
                        if (indent < 3)
                        {
                            if (isDiplomaticPact && overlordNation != null && subjectNation != null && !string.IsNullOrEmpty(pactType))
                            {
                                if (!overlordNation.DiplomaticPactByType.TryGetValue(pactType, out List<Nation>? value))
                                {
                                    value = [];
                                    overlordNation.DiplomaticPactByType[pactType] = value;
                                }

                                value.Add(subjectNation);
                            }
                            isDiplomaticPact = false;
                            subjectNation = null;
                            pactType = "";
                        }
                        if (indent < 1)
        ...
        /// <summary>
        /// Parses power bloc files and returns a list of power blocs.
        /// </summary>
        /// <param name="nations">Dictionary of nations keyed by nation tag.</param>
        /// <param name="localDir">Local directory path.</param>
        /// <returns>List of power blocs with their leader nations, members, names, and colors.</returns>
        public static List<PowerBlock> ParsePowerBlocks(Dictionary<string, Nation> nations, string localDir)
        {
            ArgumentNullException.ThrowIfNull(nations);
            ArgumentNullException.ThrowIfNull(localDir);

            string filePath = Path.Combine(localDir, "_Input", "common", "history", "power_blocs");
            List<PowerBlock> powerBlocks = [];
            foreach (string file in Directory.GetFiles(filePath, "*.txt"))
            {
                string[] lines = File.ReadAllLines(file);
                int indent = 0;
                PowerBlock? pb = null;
                Boolean inCreatePowerBlock = false;
                foreach (string line in lines)
                {
                    if (string.IsNullOrWhiteSpace(line) || line.StartsWith('#')) continue;
                    string cl = CleanLine(line);
                    ...
                    {
                        string nationTag = cl.Split("=")[0].Replace("c:", "").Replace("?","").Trim();
                        if (nations.TryGetValue(nationTag, out Nation? n))
                        {
                            pb = new PowerBlock(n);
                            powerBlocks.Add(pb);
                        }
                    ...
                    if (indent == 2 && pb != null)
                    {
                        if (cl.Contains("create_power_bloc")) inCreatePowerBlock = true;
                    }
                    if (inCreatePowerBlock && pb != null)
                    {
                        if (cl.StartsWith("name"))
                        {
                            pb.Name = cl.Split('=')[1].Trim().Replace("\"", "");
                        }
                        else if (cl.StartsWith("map_color"))
                        {
                            pb.Color = LineToColor(cl);

                        }
                        else if (cl.StartsWith("member"))
                        {
                            string nationTag = cl.Split('=')[1].Replace("c:", "").Trim();
                            if (nations.TryGetValue(nationTag, out Nation? n))
                            {
        ...
        private static Dictionary<string, object> SetDefaultConfig()
        {
            var defaultRgoColors = new List<(List<string> rgoNames, Color hColor, Color tColor)> {
                ...
                (new List<string> { "coffee", "ranch" }, Color.FromArgb(139, 69, 19), Color.FromArgb(50, 205, 50)),
